...
class PageCell : UICollectionViewCell {
   ...
   private let MyImgView : UIImageView = {
        let Img = UIImageView(image: #imageLiteral(resourceName: "onboarding_1"))
    ...
    private let descriptionTextView : UITextView = {
        ...
        let attributedtext = NSMutableAttributedString(string: "Join us today and live the life you always dream to have", attributes: [NSAttributedString.Key.font : UIFont.boldSystemFont(ofSize: 18)])
        attributedtext.append(NSMutableAttributedString(string: "\n\n\nTogether we will achieve a lot of nice things and one day we will be considered as successful people because we worked harder than any one else in this earth :)", attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 13),NSAttributedString.Key.foregroundColor: UIColor.gray]))
