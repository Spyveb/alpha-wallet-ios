...
class SwipeVC: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    let previousButton: UIButton = {
        ...
        button.addTarget(SwipeVC.self, action: #selector(PreviousButtonTapped), for: .touchUpInside)
        return button
    }()
    ...
    let nextButton: UIButton = {
        ...
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 14)
        button.setTitleColor(UIColor.gray, for: .normal)
        button.addTarget(SwipeVC.self, action: #selector(NextButtonTapped), for: .touchUpInside)
        return button
    }()
