...
class Features {
    static let `default`: Features = Features()!
    ...
    init?(fileName: String = "Features.json") {
        do {
    ...
    func isAvailable(_ item: FeaturesAvailable) -> Bool {
    ...
    func setAvailable(_ item: FeaturesAvailable, _ value: Bool) {
        guard isMutationAvailable else { return }
        featuresDictionary[item] = value
        writeToFileUrl()
    }

    func invert(_ item: FeaturesAvailable) {
...
enum FeaturesAvailable: String, CaseIterable, Codable {
